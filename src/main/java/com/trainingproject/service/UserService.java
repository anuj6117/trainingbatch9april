	...
	public Map<String,Object> addUsers(User user) {
		...
		if(user.getUserName() != null) {
			...
			if(user.getEmail() != null) {
		    	if(!(Pattern.compile("^[_-]{0,1}+[a-z0-9]+(\\_[a-z0-9]+)*[a-zA-Z]+[_-]{0,1}+(\\.[_a-z0-9-]+)*@[a-z0-9]+(\\.[a-z0-9]+)*(\\.[a-z]{2,})$").matcher(user.getEmail()).matches())) {
		    		result.put("isSuccess", false);
		...
		if(user != null && user.getStatus() == Status.ACTIVE) {
			if(role != null) {
				...
				return tempUser;
			      ...
			      throw new NullPointerException("User id does not exist or User not ACTIVE.");
		        }
	}
