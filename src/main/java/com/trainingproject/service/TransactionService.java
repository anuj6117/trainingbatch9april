...
public class TransactionService {
		...
		Integer amount=0, shadowAmount=0;
		Currency currency;
		User user;
		Wallet wallet;
				...
				if(sellers.isEmpty()) {
					for(UserOrder allBuyers:buyers) {
					  ...
					  if(currency.getInitialSupply() >= allBuyers.getCoinQuantity()) {
						...
						transaction.setExchangeRate(allBuyers.getCoinQuantity() * allBuyers.getPrice());
						...
						transaction.setStatus(Status.APPROVED);
						transaction.setMessage("Transaction is done");
						transaction.setExchangeRate(currency.getInitialSupply() * allBuyers.getPrice());
						transactionRepository.save(transaction);
				...
				if(currency != null) {
				  if(allBuyers.getPrice() >= allSellers.getPrice() && allSellers.getPrice() < currency.getPrice()
					 ...
					 if(allBuyers.getCoinQuantity() >= allSellers.getCoinQuantity()) {
						...
						transaction.setExchangeRate(allSellers.getCoinQuantity() * allBuyers.getPrice());
						transactionRepository.save(transaction);
